...
purpose: > Use Case Analysis final untuk Analytics & AI Engine (Block 7) — 26 use cases across 8 kategori.
  ...
  Dipetakan dari FIT041 Requirements (20 requirements) + FIT041 Use Case Analysis (3 UCs) + DCIM-Wiki Reference Design + Technical Requirements.
---

# Use Case Analysis — Analytics & AI Engine (Block 7) (Final)

> **Purpose:** Use Case Analysis final untuk Analytics & AI Engine (Block 7) — merged dari FIT041 Requirements + FIT041 Use Case Analysis + DCIM-Wiki Reference Design & Technical Requirements.
...
> **Merge Source:** FIT041 Technical Requirements (Jan 2026) + FIT041 Use Case Analysis (Jan 2026) + DCIM-Wiki Block 7 Reference Design (Jun 2026) + DCIM-Wiki Block 7 Technical Requirements (Jun 2026)
> **Traceability:** 20 FIT041 requirements + 3 FIT041 UCs (Use Case Analysis) mapped ke 26 use cases DCIM-Wiki
...
### FIT041 Use Case Analysis Merge

> **FIT041 Use Case Analysis (IF-Use_Case_Analysis_Analytics_AI_Engine-FIT041-20260121.md)** menyediakan 3 use case dengan detail actors, pre-conditions, flow, dan success criteria. Ketiga UC ini sudah ter-absor ke DCIM-Wiki UCs:

| FIT041 UC | Goal | Mapped to DCIM-Wiki UC | FIT041 Success Criteria |
|-----------|------|------------------------|------------------------|
| **UC1**: Predictive Failure Alerting | Peringatan dini probabilitas kegagalan aset kritikal | UC8 (Failure Probability Prediction) | Alert minimal 24–48 jam sebelum kegagalan aktual |
| **UC2**: Capacity Optimization Recommendation | Identifikasi underutilized resources dan rekomendasi konsolidasi | UC15 (Trend Analysis / Capacity) | Penghematan kapasitas ≥10% dalam satu kuartal |
| **UC3**: Energy Anomaly Detection and PUE Drift | Deteksi anomali konsumsi energi dan penyimpangan PUE real-time | UC18 (PUE Calculation) | Anomali >5% terdeteksi dalam ≤15 menit |

> LLM Reasoning Layer (FIT041) → UC24 (Natural Language Query) — LLM mengkorelasikan hasil model dengan konteks aset
...
> **FIT041 Use Case Traceability:**
> Mapped from FIT041 Use Case Analysis UC1 — **Predictive Failure Alerting for Critical Assets**
>
> | Aspect | FIT041 Detail |
> |--------|--------------|
> | **Goal** | Peringatan dini probabilitas kegagalan aset kritikal |
> | **Actors** | AI & LLM Intelligence Engine, Data Ingestion Layer, NetBox, Monitoring Dashboard |
...
> | **Flow** | (1) Data Ingestion — AI Engine menerima telemetry dan metadata aset dari NetBox dan monitoring system → (2) Feature Engineering — Sistem menghitung statistical features seperti moving average, variance, trend deviation → (3) Model Scoring — Model prediktif (LSTM/Gradient Boosting/Random Forest) menghitung probabilitas kegagalan → (4) LLM Reasoning Layer — LLM mengkorelasikan hasil model dengan konteks aset (lokasi, criticality, maintenance history) → (5) Alert Generation — Jika threshold terlampaui, alert dikirim ke dashboard dan sistem incident management → (6) Output — Alert dialihkan ke Monitoring Dashboard DCIM dan Incident Management System |
> | **Success Criteria** | Alert minimal **24–48 jam** sebelum kegagalan aktual |
>
> **DCIM-Wiki Enhancement:** UC8 menambahkan LSTM+Prophet ensemble (vs FIT041 LSTM/GB/RF), maintenance window calculation, contributing factors dengan weights, dan Kafka-based prediction stream.
...
> **FIT041 Use Case Traceability:**
> Mapped from FIT041 Use Case Analysis UC2 — **Capacity Optimization Recommendation**
>
> | Aspect | FIT041 Detail |
> |--------|--------------|
> | **Goal** | Identifikasi underutilized resources dan rekomendasi konsolidasi |
> | **Actors** | Capacity Planning Tool, AI & LLM Engine, NetBox |
...
> | **Flow** | (1) Data Retrieval — Mengambil data CPU, RAM, Storage dan rack occupancy dari NetBox & metrics store → (2) Clustering Analysis — Sistem mengelompokkan workload dengan pola pemanfaatan rendah atau saling melengkapi → (3) Optimization Modeling — AI mensimulasikan skenario konsolidasi dan dampaknya terhadap performa dan power → (4) LLM Recommendation Layer — LLM menyajikan rekomendasi dalam bentuk narasi teknis yang mudah dipahami → (5) Output — Laporan rekomendasi kapasitas, target aset, dan estimasi efisiensi |
> | **Success Criteria** | Penghematan kapasitas **≥10%** dalam satu kuartal |
>
> **DCIM-Wiki Enhancement:** UC15 menambahkan Prophet/ARIMA/Linear model selection otomatis, 7 resource types, 30-day & 365-day forecast horizons, dan confidence intervals 95%.
...
> **FIT041 Use Case Traceability:**
> Mapped from FIT041 Use Case Analysis UC3 — **Energy Anomaly Detection and PUE Drift**
>
> | Aspect | FIT041 Detail |
> |--------|--------------|
> | **Goal** | Deteksi anomali konsumsi energi dan penyimpangan PUE real-time |
> | **Actors** | Power Monitoring System, AI & LLM Engine, DCIM Operator |
> | **Trigger** | Streaming data power, suhu, dan environment |
> | **Pre-conditions** | Baseline PUE dan pola konsumsi energi telah ditentukan |
> | **Flow** | (1) Real-Time Data Ingestion — Mengambil data power dari PDU, UPS, dan sensor lingkungan → (2) Baseline Comparison — Menggunakan model statistik atau ML (Z-score, Isolation Forest) → (3) PUE Calculation — PUE dihitung secara kontinu berdasarkan power facility vs IT load → (4) Anomaly Alert — Sistem mendeteksi spike atau drift di luar batas toleransi → (5) Contextual Alerting — Alert menyertakan lokasi, perangkat terdampak, dan estimasi dampak energi → (6) Output — Alert mencakup besarnya penyimpangan dan perangkat atau area yang terpengaruh |
> | **Success Criteria** | Anomali >5% terdeteksi dalam **≤15 menit** |
>
> **DCIM-Wiki Enhancement:** UC18 menambahkan PUE rating classification (exceptional → critical), overhead percentage calculation, annual cost impact, dan hourly batch + real-time dual-mode.
...
> **FIT041 Use Case Traceability:**
> Mapped from FIT041 Use Case Analysis — **LLM Reasoning Layer** (referenced across UC1, UC2, UC3)
>
> | Aspect | FIT041 Detail |
> |--------|--------------|
> | **Context** | FIT041 Use Case Analysis mendefinisikan LLM Reasoning Layer sebagai komponen yang mengkorelasikan hasil model ML dengan konteks aset (lokasi, criticality, maintenance history) dan menyajikan rekomendasi dalam bentuk narasi teknis yang mudah dipahami |
> | **Integration Points** | UC1 (Failure Prediction) → LLM mengkorelasikan probabilitas kegagalan dengan konteks aset; UC2 (Capacity) → LLM menyajikan rekomendasi konsolidasi; UC3 (Energy) → LLM memberikan contextual alerting |
> | **LLM Backends** | Ollama / Local LLM (FIT041 requirement) — DCIM-Wiki supports GPT-4, Claude, Local LLM (configurable) |
>
> **DCIM-Wiki Enhancement:** UC24 menambahkan RAG retrieval multi-source (CMDB, logs, runbooks, metrics), intent classification, hallucination validation, dan citation mechanism. FIT041 LLM Reasoning Layer ter-absor ke UC24 + UC25 (Explanation Generation).
...
| **UC8** | ✅ Daily batch ≥ 95% CIs covered<br>✅ Failure probability dalam 0.0–1.0<br>✅ Contributing factors dengan weights<br>✅ Maintenance window calculated<br>✅ Predictions terpublish ke Kafka<br>✅ **FIT041:** Alert minimal 24–48 jam sebelum kegagalan aktual |
...
| **UC15** | ✅ 7 resource types analyzed<br>✅ Model selection otomatis<br>✅ 30-day forecast dengan CI 95%<br>✅ MAPE < 10%<br>✅ Results ke Kafka + dashboard<br>✅ **FIT041:** Penghematan kapasitas ≥10% dalam satu kuartal |
...
| **UC18** | ✅ PUE calculated hourly<br>✅ PUE rating assigned<br>✅ Overhead percentage calculated<br>✅ Historical trend available<br>✅ Alert fires jika PUE > 1.6<br>✅ **FIT041:** Anomali >5% terdeteksi dalam ≤15 menit |
...
### 19.3 FIT041 Use Case Analysis Success Criteria

| Metric | FIT041 Target | Applicable UC | Status |
|--------|--------------|---------------|--------|
| Predictive failure alert lead time | ≥ 24–48 jam before actual failure | UC8 | ✅ Mapped |
| Capacity savings per quarter | ≥ 10% | UC15 | ✅ Mapped |
| Energy anomaly detection latency | ≤ 15 menit | UC18 | ✅ Mapped |
| AI Services uptime | ≥ 99% | All AI components | ✅ Covered by SLA |
| LLM inference backend | Ollama / Local LLM | UC24 | ✅ Supported (configurable) |
| ML framework support | PyTorch, TensorFlow | UC21, UC23 | ✅ Supported |
| Concurrent model support | Multiple models simultaneously | UC23 | ✅ Supported |
...
| UC18 PUE Calculation | — (FIT041 missing) → **FIT041 UC3** | **Merged** | PUE formula, rating, cost impact, alerting + FIT041 UC3 (Energy Anomaly Detection & PUE Drift) |
...
| UC24 NL Query | — (FIT041 missing) → **FIT041 LLM Reasoning Layer** | **Merged** | RAG retrieval, multi-source, citations + FIT041 LLM Reasoning Layer context |
...
## 21. FIT041 Use Case Analysis Merge Detail

> **Source:** IF-Use_Case_Analysis_Analytics_AI_Engine-FIT041-20260121.md
> **Date:** 2026-01-21
> **UCs Absorbed:** 3 (Failure Prediction, Capacity Optimization, Energy Anomaly Detection)

### 21.1 UC1: Predictive Failure Alerting for Critical Assets → UC8

| FIT041 Element | Detail | DCIM-Wiki UC8 Equivalent |
|----------------|--------|--------------------------|
| **Goal** | Peringatan dini probabilitas kegagalan aset kritikal | ✅ Predict failure probability dalam 30-day window |
| **Actors** | AI & LLM Intelligence Engine, Data Ingestion Layer, NetBox, Monitoring Dashboard | ✅ Predictive Maintenance Service, IT Operations, DC Managers, Workflow Automation |
| **Trigger** | Streaming real-time data operasional (temperature, power draw, SMART disk, fan speed, utilization) | ✅ Daily batch (02:00 UTC) — enhancement: adds real-time streaming capability |
| **Pre-conditions** | Model telah dilatih menggunakan data historis kegagalan dan kondisi normal | ✅ LSTM model trained + Historical metrics ≥ 90 days |
| **Flow** | (1) Data Ingestion → (2) Feature Engineering → (3) Model Scoring → (4) LLM Reasoning Layer → (5) Alert Generation → (6) Output | ✅ 10-step flow: Schedule → Collect → Predict (LSTM) → Predict (Prophet) → Combine → Classify → Enrich → Schedule Maintenance → Emit → Store |
| **Success Criteria** | Alert minimal 24–48 jam sebelum kegagalan aktual | ✅ Daily batch predictions + maintenance window calculation |
| **Not in FIT041** | — | Ensemble model (LSTM 0.7 + Prophet 0.3), contributing factors, Kafka streaming |

### 21.2 UC2: Capacity Optimization Recommendation → UC15

| FIT041 Element | Detail | DCIM-Wiki UC15 Equivalent |
|----------------|--------|--------------------------|
| **Goal** | Identifikasi underutilized resources dan rekomendasi konsolidasi | ✅ Trend analysis + UC16 projection + UC17 alerting |
| **Actors** | Capacity Planning Tool, AI & LLM Engine, NetBox | ✅ Capacity Forecasting Service, DC Managers, IT Operations |
| **Trigger** | Analisis terjadwal (mingguan) atau permintaan ad-hoc | ✅ Daily batch (02:30 UTC) — enhanced frequency |
| **Pre-conditions** | Data kapasitas dan utilisasi tersedia minimal 90 hari | ✅ Historical metrics ≥ 30 days + 90 days for model training |
| **Flow** | (1) Data Retrieval → (2) Clustering Analysis → (3) Optimization Modeling → (4) LLM Recommendation Layer → (5) Output | ✅ 8-step flow: Schedule → Collect → Model Selection → Fit → Project → Confidence → Store → Emit |
| **Success Criteria** | Penghematan kapasitas ≥10% dalam satu kuartal | ✅ UC16 generates optimization recommendations + cost impact |
| **Not in FIT041** | — | Prophet/ARIMA/Linear model selection, 7 resource types, 30/365-day horizons, confidence intervals |

### 21.3 UC3: Energy Anomaly Detection and PUE Drift → UC18

| FIT041 Element | Detail | DCIM-Wiki UC18 Equivalent |
|----------------|--------|--------------------------|
| **Goal** | Deteksi anomali konsumsi energi dan penyimpangan PUE real-time | ✅ PUE calculation + anomaly alerting |
| **Actors** | Power Monitoring System, AI & LLM Engine, DCIM Operator | ✅ Energy Optimization Service, Facilities Team, DC Managers |
| **Trigger** | Streaming data power, suhu, dan environment | ✅ Hourly batch + real-time |
| **Pre-conditions** | Baseline PUE dan pola konsumsi energi telah ditentukan | ✅ BMS/EPMS data + Historical PUE ≥ 30 days |
| **Flow** | (1) Real-Time Data Ingestion → (2) Baseline Comparison → (3) PUE Calculation → (4) Anomaly Alert → (5) Contextual Alerting → (6) Output | ✅ 8-step flow: Collect → Calculate → Rate → Overhead → Cost → Store → Alert → Emit |
| **Success Criteria** | Anomali >5% terdeteksi dalam ≤15 menit | ✅ Hourly PUE calculation + alert when PUE > 1.6 |
| **Not in FIT041** | — | PUE rating classification, overhead percentage, annual cost impact, energy dashboard |

### 21.4 LLM Reasoning Layer → UC24 + UC25

| FIT041 Element | Detail | DCIM-Wiki Equivalent |
|----------------|--------|----------------------|
| **Context** | LLM mengkorelasikan hasil model dengan konteks aset (lokasi, criticality, maintenance history) | ✅ UC24 (NL Query) + UC25 (Explanation Generation) |
| **Backends** | Ollama / Local LLM | ✅ GPT-4, Claude, Local LLM (configurable) |
| **Usage in UC1** | LLM mengkorelasikan probabilitas kegagalan dengan konteks aset | ✅ UC25 explains predictions |
| **Usage in UC2** | LLM menyajikan rekomendasi dalam bentuk narasi teknis | ✅ UC25 explains capacity forecasts |
| **Usage in UC3** | Alert menyertakan lokasi, perangkat terdampak, estimasi dampak energi | ✅ UC24 NL queries + UC25 explanations |

### 21.5 FIT041 Requirements Checklist (from Use Case Analysis)

| # | Requirement | Status | Evidence |
|---|-------------|--------|----------|
| 1 | Technical: Integrasi dengan NetBox API & metrics store | ✅ Covered | UC8/UC15/UC18 data flows |
| 2 | Technical: Support ML framework (PyTorch, TensorFlow) | ✅ Covered | UC21 training pipeline |
| 3 | Technical: LLM inference via Ollama / Local LLM | ✅ Covered | UC24 multi-backend |
| 4 | Performance: Pemrosesan real-time telemetry data | ✅ Covered | UC1 (<1s), UC4 (<500ms) |
| 5 | Scalability: Support multiple concurrent models | ✅ Covered | UC23 A/B testing, UC22 registry |
| 6 | Data Quality: Deteksi & penanganan noisy/incomplete data | ✅ Covered | UC7 (FP handling), DQ rules |
| 7 | Availability: Uptime AI Services ≥ 99% | ✅ Covered | SLA targets 99.9% |
...
- [[IF-Use_Case_Analysis_Analytics_AI_Engine-FIT041-20260121]] — FIT041 Use Case Analysis (source — 3 UCs)
...
> **Status:** Final — Merged dari FIT041 Requirements + FIT041 Use Case Analysis + DCIM-Wiki Reference Design + Technical Requirements
> **Date:** 2026-06-25
> **Purpose:** Use Case Analysis final untuk Analytics & AI Engine (Block 7) — 26 use cases across 8 kategori
...
> **FIT041 Use Case Analysis:** 3 UCs merged (Failure Prediction, Capacity Optimization, Energy Anomaly Detection)
